...
// mongoDB connection
// const uri =
//   "mongodb+srv://moudjedmouhamed31:1s6cRDXjeeU1VzGZ@cluster0.ldrmcu9.mongodb.net/ClassCompanion?retryWrites=true&w=majority";
const uri =
  "mongodb+srv://moudjedmouhamed31:1s6cRDXjeeU1VzGZ@cluster0.gonjvh9.mongodb.net/ClassCompanion?retryWrites=true&w=majority";
...
app.post("/admin", (req, res) => {
  ...
  admin
    ...
    .then((result) => {
      console.log(result);
      res.send({ dbresult: result, account_type: "admin" });
    })
    .catch((err) => {
      console.log(err);
      res.send({ message: "Failed to create admin account" });
    });
});
...
app.post("/addsesion",(req,res)=>{
  console.log(req.body.sesiondate)
   const sesion = new Sesion({
   teacher_id : req.body.teacher_id,
   sesiondate :req.body.sesiondate,
...
app.get("/studentsbyGroup/:sesionGroup/:speciality", (req, res) => {
  const {sesionGroup,speciality}=req.params;
  Student.find(
    { group: sesionGroup, speciality },
    (err, students) => {
      if (err || students === null) {
        res.send({ message: "Error, could not get students" });
      } else {
