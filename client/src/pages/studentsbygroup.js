...
function Studentsbygroup() {
  ...
  const sesionGroup= location.state.sesionGroup;
  const speciality=location.state.speciality;
  useEffect(() => {
    if (!loggedIn) {
      navigate("/login");
    } else {
      setIsLoading(true);
      setTimeout(() => {
        Axios.get(`http://localhost:3001/studentsbyGroup/${sesionGroup}/${speciality}`)
          .then((res) => {
